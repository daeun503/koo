...
class Chunk(TimestampMixin, Base):
    ...
    context_id = Column(Integer, nullable=True, comment="청크가 속한 컨텍스트 ID")
    ...
    token_len = Column(Integer, nullable=False, default=0, comment="청크 토큰 길이")
...
class Document(SoftDeleteMixin, TimestampMixin, Base):
    ...
    raw_content = Column(Text, nullable=False, comment="문서 내용")
...
class QueryLog(TimestampMixin, Base):
    ...
    router_domain = Column(Enum(RouterDomain), nullable=False, comment="라우터 도메인")
