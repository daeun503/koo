...
from app.repository.llm import Answerer as AnswererRepository
from app.repository.llm import Embedder as EmbedderRepository
...
class OpenaiAnswerer(AnswererRepository):
    ...
    def answer(self, question: str, contexts: list[dict]) -> str:
        ...
        # 사람이 보기 좋은 렌더링
        used = ", ".join([str(i) for i in out.sources]) if out.sources else "none"
        return f"{out.answer}\n\n(sources: {used}, confidence: {out.confidence:.2f})"
